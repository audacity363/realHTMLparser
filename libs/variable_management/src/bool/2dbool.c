...
int new2DBoolean(VariableObject *anker, char *group, char *name, int x_length, int y_length)
{
    ...
    if((target = addNewVariable(anker, group, name, TWODBOOL)) == NULL)
        return(-1);

    target->size = (sizeof(bool)*x_length)*y_length;
...
int set2DBooleanXY(VariableObject *anker, char *group, char *name, int x, int y, bool val)
{
    ...
    if(target->type != TWODBOOL)
    {
        rh4n_errno = WRONG_VAR_TYPE;
        return(-1);
    }
    
    *((bool*)target->data+OFFSET_2DBoolean(target->array_length[0], x, y)) = val;
    return(0);
}
...
bool get2DBooleanXY(VariableObject *anker, char *group, char *name, int x, int y)
{
    ...
    return(*((bool*)(target->data+OFFSET_2DBoolean(target->array_length[0], x, y))));
}
...
int print2DBoolean(VariableObject *target, FILE *output, int mode)
{
    ...
    for(; i < target->array_length[0]; i++)
    {
        ...
        {
            offset = OFFSET_2DBoolean(target->array_length[0], i, x);
            fprintf(output, "%s", 
